...
import { apiDocsHtml } from './api-docs.mjs'
...
// core JSON-RPC API reference at /api-docs/ — generated by api-docs.mjs from
// build/core (the pinned submodule WITH patches/core applied), so it documents
// the API this bundle actually runs rather than the nearest published
// @deltachat/jsonrpc-client release. Self-contained page, no CDN, like the
// changelog viewer; kept out of the SW precache (instance-config precacheSkip).
// No routes.caddy entry: file_server already serves it, and unlike dist/caddy/
// (which gets `error /caddy/* 404` because it must NOT leave the box) this is
// meant to be public.
await mkdir(join(dist, 'api-docs'))
const corePatches = (await readdir(join(repo, 'patches/core'))).filter(f => f.endsWith('.patch')).length
await writeFile(join(dist, 'api-docs/index.html'), await apiDocsHtml(join(repo, 'build/core'), corePatches))
