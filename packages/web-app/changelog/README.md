...
`web.slothful.chat/changelog`). It shows the release notes for the web app
itself plus the three npm packages this project publishes:
`@slothfulchat/core-wasm`, `@slothfulchat/ws-tcp-proxy` and
...
the same single-page viewer, cut down to our clients and pointed at local
markdown instead of pulling live from GitHub/Codeberg.
...
drops the web app's and each package's `CHANGELOG.md` in beside it as
`<name>.md` (`web-app.md`, `core-wasm.md`, `ws-tcp-proxy.md`, `customize.md`).
The page then `fetch`es
