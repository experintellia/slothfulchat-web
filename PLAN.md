...
5. **In scope** (beyond basic chat): **multiaccount** (core is account-manager based — nearly free) and **file-based IMEX** (backup export/import + key import/export via `<input type=file>` upload and blob-URL download; needs the FS shim to hold the backup file). **Descoped**: webxdc, stickers, HTML email, video calls, sqlcipher, iroh backup *transfer* (QR/LAN). Descope list is documented in `DESCOPED.md` in the repo, each entry with reason + what re-enabling would take.
...
- webxdc, stickers, HTML email, video calls (also missing in upstream's browser edition).
