...
//
// Three assertions guard the premise, all fatal:
//   1. two independently migrated accounts are equivalent — schema, every
//      table's rows, and the persistent pragmas. This is the reason ONE
//      template can serve every account. It compares two accounts made
//      milliseconds apart, so it catches per-account state, not slow-moving
//      time dependence; nothing in the migrations has either today.
//   2. the core actually stamps an account out of the template rather than
//      quietly migrating instead,
//   3. and that account matches a migrated one.
 ...
 * returns `{ accounts, seeded }`: each account's database as base64,
 * `{db, wal}`, and how many of them the core actually stamped out of the
 * template. `template` (base64) is installed before init when given.
 *
 * `seeded` is the only way to tell the fast path is live — a seeded account
 * is byte-identical to a migrated one by design, so no amount of comparing
 * databases can distinguish "seeding works" from "seeding silently stopped
 * happening". */
async function makeAccounts(browser, count, template) {
  const page = await browser.newPage()
  page.on('pageerror', e => console.error('[pageerror]', e.message))
  let seeded = 0
  page.on('console', m => m.text().includes('from the account template') && seeded++)
...
function fingerprint(path) {
  ...
  // auto_vacuum in particular is load-bearing and silent if it regresses:
  // `PRAGMA auto_vacuum=INCREMENTAL` in core's new_connection is a no-op on a
  // non-empty database, so a seeded account inherits whatever the template
  // carries — get it wrong and housekeeping stops reclaiming pages, forever,
  // with nothing to see.
  const pragmas = ['auto_vacuum', 'page_size', 'encoding'].map(
    p => `${p}=${db.prepare(`PRAGMA ${p}`).get()[p]}`
  )
  db.close()
  return JSON.stringify({ pragmas, schema, rows }, (_, v) => (typeof v === 'bigint' ? String(v) : v), 1)
}
...
try {
  ...
  if (seeded.seeded !== 1)
    throw new Error(
      'the core did not stamp the account out of the template — it fell back to ' +
        'migrating, so this build would ship the template and never use it'
    )
  if (fingerprint(await land(tmp, 'seeded', seeded.accounts[0])) !== a)
    throw new Error('an account seeded from the template differs from a migrated one')
