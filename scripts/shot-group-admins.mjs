...
try {
  ...
  for (let i = 0; i < 10; i++) {
    await version.click()
    if (i === 7) {
      // remaining === 2 → countdown toast. It must render in the top layer
      // (Popover API) so it shows ABOVE the open About dialog, not behind it.
      const toast = page
        .locator('.user-feedback')
        .filter({ hasText: 'steps away' })
        .first()
      await toast.waitFor({ state: 'visible', timeout: 5_000 })
      const inTopLayer = await toast.evaluate(el =>
        el.matches(':popover-open')
      )
      if (!inTopLayer) {
        throw new Error('countdown toast is not in the top layer (renders behind the dialog)')
      }
