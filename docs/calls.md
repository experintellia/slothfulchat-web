...
2. **Incoming**: `IncomingCall{place_call_info, has_video}` → ring overlay in main
   ...
   **Accepting answers audio-only**, whatever the caller's `has_video` said: that
   flag describes what the CALLER sends, and one generic "Accept" must not also
   consent to filming the recipient (a previously granted camera permission would
   otherwise start the camera silently). Nothing is lost — the video m-line is
   negotiated on every call, so the caller's video/screen-share still arrives and
   the recipient's own camera is one in-call button away.
