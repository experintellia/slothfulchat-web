...
// ponytail: on wasm32 the tokio facade's in-memory fs has its own Metadata
// type (std::fs is unsupported there); everywhere else use the real one.
...
impl Header {
    ...
    // ponytail: memfs has no uid/gid/mode; synthesize 0644/0755 like the
    // windows impl below. Was `unimplemented!()` upstream (wasi-only support).
    #[cfg(target_arch = "wasm32")]
