...
impl<R: Read + Unpin> Entry<R> {
    ...
    pub async fn unpack_in<P: AsRef<Path>>(&mut self, dst: P) -> io::Result<Option<PathBuf>> {
        // ponytail: tokio::fs so the wasm32 memfs facade can resolve it
        // (std::fs::canonicalize is unsupported there).
        let dst = fs::canonicalize(dst.as_ref()).await?;
...
impl<R: Read + Unpin> EntryFields<R> {
    ...
    async fn unpack(&mut self, target_base: Option<&Path>, dst: &Path) -> io::Result<Unpacked> {
        ...
        // ponytail: skipped on wasm32 — the facade's memfs File has no
        // into_std/from_std and doesn't track mtimes; harmless to drop.
        #[cfg(not(target_arch = "wasm32"))]
        if self.preserve_mtime {
            if let Some(mtime) = get_mtime(&self.header)? {
    ...
    async fn validate_inside_dst(&self, dst: &Path, file_dst: &Path) -> io::Result<()> {
        ...
        // ponytail: routed through tokio::fs so the wasm32 memfs facade can
        // resolve it (std::fs::canonicalize is unsupported there).
